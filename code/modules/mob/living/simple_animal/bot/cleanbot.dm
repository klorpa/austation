...
/mob/living/simple_animal/bot/cleanbot/handle_automated_action()
	...
	if(target)
		...
		if(loc == get_turf(target))
			if(!(check_bot(target) && prob(50)))	//Target is not defined at the parent. 50% chance to still try and clean so we don't get stuck on the last blood drop.
				...
				shuffle = TRUE	//Shuffle the list the next time we scan so we don't both go the same way.
			path = list()
